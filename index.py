...
def first_page(data):
    ...
    # Header
    # can.drawImage("./19.png",0, 720, width=615, height=72)
    can.drawImage("./22.png",0, 720, width=615, height=72)
    can.drawCentredString(300, 750, "Report")
    can.drawCentredString(500, 768, "No.20/6 Sengunthar Street,")
    can.drawCentredString(500, 754, "Shenoy Nagar (East)")
    can.drawCentredString(500, 739, "Chennai 600030, Tamil Nadu")
    can.drawImage("./empty.png",30, 590, width=550, height=92)
    width, height = A4
    # Starting position
    x_start = 13 * mm
    y_start = height - 60 * mm
    ...
    can.drawString(x_start + 45*mm, y_start - 22, data['Name'])

    can.drawString(x_start + 90*mm, y_start - 22, "Sex:")
    can.drawString(x_start + 105*mm, y_start - 22, data["sex"])  

    can.drawString(x_start + 130*mm, y_start - 22, "Investigation:")
    can.drawString(x_start + 130*mm, y_start - 44, data["investigation"])  

    # Second row
    can.drawString(x_start + 5, y_start - 44, "Medras ID:")
    can.drawString(x_start + 45*mm, y_start - 44, data["medras_id"])

    can.drawString(x_start + 90*mm, y_start - 44, "Age:")
    can.drawString(x_start + 105*mm, y_start - 44, str(data["age"]))  
...
if st.button("Download Summary"):
    ...
    if uploaded_file:
        ...
        for i, page in enumerate(reader.pages):
            overlay =  first_page(data) if i == 0 else all_pages(str(i + 1))
